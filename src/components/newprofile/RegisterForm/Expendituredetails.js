...
const ExpenditureDetails = (props) => {
  ...
  const { user } = state;
  const { monthlyExpenditure, hasLoan: initialHasLoan, loanAmount,laondura, hasInsurance: initialHasInsurance, insuranceAmount } = user;
  ...
  useEffect(() => {
    ...
    const newLoandura= hasLoan ? state.user.laondura : 0;
    ...
    setState((prevState) => ({
      ...
      user: {
        ...
        laondura: newLoandura,
        insuranceAmount : newInsuranceAmount,
      },
    }));
  }, [hasLoan, state.user.loanAmount, state.user.laondura,hasInsurance,insuranceAmount]);
      ...
      <Grid container spacing={2}>
        ...
        {hasLoan && (
          ...
          <TextField
            ...
            type="number"
            value={laondura}
            onChange={(e) => setState((prevState) => ({ ...prevState, user: { ...prevState.user, laondura: e.target.value } }))}
            required={hasLoan}
            variant="outlined"
            margin="normal"
          />
